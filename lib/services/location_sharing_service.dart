...
import 'package:hajj_companion/models/location_share.dart';
...
class LocationSharingService {
  ...
  Stream<List<MemberWithLocation>> streamMembersWithLocations() {
    ...
    return _groupService.streamMembers().asyncMap((members) async {
      ...
      for (final member in members) {
        ...
        membersWithLocations.add(MemberWithLocation(
          member: member,
          location: location,
        ));
      }
  ...
  Future<void> _checkAndAlertDistances() async {
    try {
      ...
      final currentAlertedIds = alerts.map((a) => a.childMember.deviceId).toSet();
